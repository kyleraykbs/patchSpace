...
class MainWindow(Gtk.ApplicationWindow):
    def __init__(self, app):
        ...
        # Whether this GUI wants a daemon running.  True until the user
        # explicitly stops it, so a start that failed (or a daemon that
        # later died) is retried instead of leaving the GUI permanently
        # disconnected.
        self._daemon_should_run = True
        self._daemon_busy = False
        self._next_daemon_retry_at = 0.0
        self.client = PatchBayClient()
    ...
    def _poll_daemon_connection(self):
        ...
        and goes, pull a fresh snapshot the moment it (re)connects, and -
        while this GUI wants a daemon - keep retrying to start one instead
        ...
        if running != self._daemon_connected:
            ...
            if running:
                self.pw_widget.refresh()
                self.ps_widget.refresh()
        if not running and self._daemon_should_run and not self._daemon_busy:
            now = time.monotonic()
            if now >= self._next_daemon_retry_at:
                self._next_daemon_retry_at = now + DAEMON_RETRY_INTERVAL_S
    ...
    def _daemon_action(self, action):
        ...
        # Explicit stop means "I don't want a daemon" - suppress the
        # background retry loop; start/restart (re-)enable it.
        self._daemon_should_run = action != "stop"
    ...
    def on_close(self, *args):
        # Stop the retry loop before tearing anything down.
        self._daemon_should_run = False
        if self.log_console is not None:
